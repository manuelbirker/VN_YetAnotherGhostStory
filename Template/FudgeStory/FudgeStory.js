...
(function (FudgeStory) {
    var ƒ = FudgeCore;
    class Interactable extends FudgeStory.Base {
        ...
        poses = new Map();
        origin;
        constructor(_interactable) {
            super();
            this.posX = Reflect.get(_interactable, "posX") || 0;
            this.posY = Reflect.get(_interactable, "posY") || 0;
            this.img = Reflect.get(_interactable, "img");
            this.definition = _interactable;
            this.origin = Reflect.get(_interactable, "origin") || ƒ.ORIGIN2D.BOTTOMCENTER;
            this.sizeX = Reflect.get(_interactable, "sizeX");
            this.sizeY = Reflect.get(_interactable, "sizeY");
            this.interactable = Reflect.get(_interactable, "interactable");
            this.name = Reflect.get(_interactable, "name");
            Interactable.interactables.set(_interactable.name, this);
        }
        static get(_interactable) {
            let result = Interactable.interactables.get(_interactable.name);
            return result || new Interactable(_interactable);
        }
        static getByName(_name) {
            return Interactable.interactables.get(_name);
        }
        static async show(_interactable, _pose, _position) {
            let interactable = Interactable.get(_interactable);
            let pose = await interactable.getPose(_pose);
            pose.mtxLocal.set(ƒ.Matrix4x4.TRANSLATION(_position.toVector3(0)));
            FudgeStory.Base.middle.appendChild(pose);
            await Interactable.clickable(_interactable);
        }
        static async hide(_interactable) {
            let found = FudgeStory.Base.middle.getChildrenByName(_interactable.name);
            if (found.length == 0)
                console.warn(`No interactable with name ${_interactable.name} to hide`);
            if (found.length > 1)
                console.warn(`Multiple interactable with name ${_interactable.name} exist, removing first`);
            FudgeStory.Base.middle.removeChild(found[0]);
        }
        static hideAll() {
            FudgeStory.Base.middle.removeAllChildren();
        ...
        static async clickable(_interactable) {
            let ia = _interactable;
            if (!ia.interactable) {
                return;
            }
            ...
            clickable.style.height = ia.sizeX + "px";
            clickable.style.width = ia.sizeY + "px";
            clickable.style.top = 50 + ia.posY + "%";
            clickable.style.left = 50 + ia.posX + "%";
            clickable.style.backgroundColor = "red";
            clickable.style.zIndex = "9999";
            clickable.style.cursor = "pointer";
            clickable.className = "clickable " + ia.name;
            function hndClick(_event) {
                clickable.removeEventListener("click", hndClick);
                document.body.removeChild(clickable);
                if (clickable.classList.contains(ia.name)) {
                    let found = FudgeStory.Base.middle.getChildrenByName(ia.name);
                    FudgeStory.Base.middle.removeChild(found[0]);
                }
            }
            clickable.addEventListener("click", hndClick);
        }
    }
    FudgeStory.Interactable = Interactable;
})(FudgeStory || (FudgeStory = {}));
var FudgeStory;
(function (FudgeStory) {
    // import ƒ = FudgeCore;
...
(function (FudgeStory) {
    ...
    class Sound {
        ...
        static stop(_url, _volume, _loop = false) {
            let sound = Sound.sounds.get(_url);
            sound = new Sound(_url, _loop);
            sound.cmpAudio.volume = 0;
            sound.cmpAudio.play(false);
            return sound;
        }
        /**
...
(function (FudgeStory) {
    ...
    class Transition extends FudgeStory.Base {
        ...
        static async get(_url) {
            ...
            crcTransition.drawImage(txtTransition.image, 0, 0, txtTransition.image.width, txtTransition.image.height, 0, 0, 1920, 1080); // Temporary Resolution Fix vor Full HD Resolutions
            transition = crcTransition.getImageData(0, 0, 1920, 1080).data;
